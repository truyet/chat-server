...
public class AuthService {
  ...
  public TokenInfo login(String userName, String password) {
    ...
    if (user.isEmpty()) {
      var newUser = User.builder().username(userName).password(PasswordUtils.passwordHash(password))
          .build();
    ...
    JWTClaimsSet claimsSet = new JWTClaimsSet.Builder()
        ...
        .build();
    try {
      return TokenInfo.builder().accessToken(jwtUtils.encode(claimsSet))
          .expiredAt(expireAt.getTime()).build();
    } catch (Exception e) {
